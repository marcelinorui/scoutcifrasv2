...
export function stripCharsInRE (s, bag)
...
export function stripCharsInBag (s, bag)
...
export function stripCharsNotInBag (s, bag)
...
export function charInString (c, s)
...
export function stripInitialWhitespace (s)
...
export function isInteger (s, eok?)
{   
...
export function isSignedInteger (s, eok?)
...
export function isPositiveInteger (s, eok?)
...
export function isNonnegativeInteger (s, eok?)
{   let secondArg = defaultEmptyOK;
...
export function isNegativeInteger (s, eok?)
{   let secondArg = defaultEmptyOK;
...
export function isNonpositiveInteger (s, eok?)
{   let secondArg = defaultEmptyOK;
...
export function isSignedFloat (s, eok?)
...
export function isAlphabetic (s, eok?)
...
export function isAlphanumeric (s, eok?)
...
export function reformat (s, ...args:any[])
...
export function isUSPhoneNumber (s, eok?)
{   if (isEmpty(s)) 
...
export function isInternationalPhoneNumber (s, eok?)
{   if (isEmpty(s)) 
...
export function isZIPCode (s, eok?)
{  if (isEmpty(s)) 
...
export function isStateCode(s, eok?)
{   if (isEmpty(s)) 
...
export function isIntegerInRange (s, a, b, eok?)
{   if (isEmpty(s)) 
...
export function daysInFebruary (year)
...
export function isDate (year, month, day)
...
export function warnEmpty (theField, s)
{   theField.focus()
...
export function warnInvalid (theField, s)
{   theField.focus()
    theField.select()
    alert(s)
    return false
...
export function checkString (theField, s, emptyOK)
...
export function checkStateCode (theField, emptyOK)
...
export function reformatZIPCode (ZIPString)
...
export function checkZIPCode (theField, emptyOK)
{   if (checkZIPCode.arguments.length == 1) emptyOK = defaultEmptyOK;
    if ((emptyOK == true) && (isEmpty(theField.value))) return true;
    else
    { let normalizedZIP = stripCharsInBag(theField.value, ZIPCodeDelimiters)
...
export function reformatUSPhone (USPhone)
...
export function checkUSPhone (theField, emptyOK)
{   if (checkUSPhone.arguments.length == 1) emptyOK = defaultEmptyOK;
    if ((emptyOK == true) && (isEmpty(theField.value))) return true;
    else
    {  let normalizedPhone = stripCharsInBag(theField.value, phoneNumberDelimiters)
...
export function checkInternationalPhone (theField, emptyOK)
...
export function checkEmail (theField, emptyOK)
...
export function reformatSSN (SSN)
...
export function checkSSN (theField, emptyOK)
{   if (checkSSN.arguments.length == 1) emptyOK = defaultEmptyOK;
    if ((emptyOK == true) && (isEmpty(theField.value))) return true;
    else
    {  let normalizedSSN = stripCharsInBag(theField.value, SSNDelimiters)
...
export function checkYear (theField, emptyOK)
...
export function checkMonth (theField, emptyOK)
...
export function checkDay (theField, emptyOK)
...
export function checkDate (yearField, monthField, dayField, labelString, OKtoOmitDay)
{   // Next line is needed on NN3 to avoid "undefined is not a number" error
    // in equality comparison below.
...
export function checkCreditCard (radio, theField)
{   let cardType = getRadioButtonValue (radio)
    let normalizedCCN = stripCharsInBag(theField.value, creditCardDelimiters)
...
export function isCreditCard(st) {
  // Encoding only works on cards with less than 19 digits
  if (st.length > 19)
    return (false);

  let sum = 0; let mul = 1;let l = st.length;
  for (let i = 0; i < l; i++) {
    let digit = st.substring(l-i-1,l-i);
    let tproduct = parseInt(digit ,10)*mul;
    if (tproduct >= 10)
      sum += (tproduct % 10) + 1;
    else
      sum += tproduct;
...
export function isMasterCard(cc)
{
  let firstdig = cc.substring(0,1);
  let seconddig = cc.substring(1,2);
...
export function isAmericanExpress(cc)
{
  let firstdig = cc.substring(0,1);
  let seconddig = cc.substring(1,2);
...
export function isDinersClub(cc)
{
  let firstdig = cc.substring(0,1);
  let seconddig = cc.substring(1,2);
...
export function isCarteBlanche(cc)
{
...
export function isDiscover(cc)
{
  let first4digs = cc.substring(0,4);
...
export function isCardMatch (cardType, cardNumber)
{
...
export function IsDC (cc)  {
  return isDinersClub(cc);
}

export function IsDiners (cc)  {
  return isDinersClub(cc);
}
