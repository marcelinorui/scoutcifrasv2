...
import { RemoveDiacritics } from "../../app/helper/diacritics";
import { isNonnegativeInteger, isYear, isInteger, isLetter, isIntegerInRange, isAlphabetic } from "../../app/helper/validation";
...
export abstract class CifraBase implements ICifra {
    ...
    protected GetInteger(n: string | number): number {
    ...
    protected GetYear(n: string | number): number{
        if(isYear(n)){
            return Number(n);
        }
        return 1942;
    ...
    protected GetLetter(n: string | number): string{
        if(isLetter(n)){
    ...
    protected GetIntRange(n:string|number, min:number, max:number){
    ...
    protected GetAlphabetic(n:string|number){
        if(isAlphabetic(n)){
            return n;
        }
        return '';
    ...
    public GetHelpTemplate(service:ScoutCifrasService):void {
